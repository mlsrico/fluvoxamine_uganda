...
#data <- readRDS("data/filtered_sample_outcomes.rds") %>%
data <- read_excel("revision code/data/fluvoxamine_data ms.xlsx") %>% 
      mutate(vaccinated_bin = vaccinated != "No") %>% 
      #filter(!death_status) %>% 
      filter(death_status=="FALSE") %>% 
      droplevels()

# Table - N(%) / Mean (SD) -------
tdata <- data %>% mutate_if(is.logical, ~factor(as.character(.), levels = c("TRUE", "FALSE")))
...
pdata <- data 
ps <- glm(fluvoxamine ~ ., data = pdata %>% 
                select(all_of(vars_for_ps), 
                       -any_comorb, -vaccinated_bin, 
                       -any_symptom), 
          family = "binomial")
...
saveRDS(ps_dat, "revision code/data/ps_data_survivors.rds")
